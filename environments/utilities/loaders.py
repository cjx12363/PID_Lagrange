...
from environments.models.ev_charger import EV_Charger
from environments.models.ev import EV
from environments.models.transformer import Transformer

from environments.utilities.utils import EV_spawner, generate_power_setpoints, EV_spawner_GF
...
def load_ev_spawn_scenarios(env) -> None:
    ...
    if env.config['heterogeneous_ev_specs']:
        ...
        if "ev_specs_file" in env.config:
            ...
            ev_specs_file = os.path.join(_data_dir, 'ev_specs.json')
    ...
    df_arrival_week_file = os.path.join(_data_dir, 'distribution-of-arrival.csv')
    df_arrival_weekend_file = os.path.join(_data_dir, 'distribution-of-arrival-weekend.csv')
    df_connection_time_file = os.path.join(_data_dir, 'distribution-of-connection-time.csv')
    df_energy_demand_file = os.path.join(_data_dir, 'distribution-of-energy-demand.csv')
    time_of_connection_vs_hour_file = os.path.join(_data_dir, 'time_of_connection_vs_hour.npy')

    df_req_energy_file = os.path.join(_data_dir, 'mean-demand-per-arrival.csv')
    df_time_of_stay_vs_arrival_file = os.path.join(_data_dir, 'mean-session-length-per.csv')
...
def generate_residential_inflexible_loads(env) -> np.ndarray:
    ...
    # Load the data
    data_path = os.path.join(_data_dir, 'residential_loads.csv')
...
def generate_pv_generation(env) -> np.ndarray:
    ...
    # Load the data
    data_path = os.path.join(_data_dir, 'pv_netherlands.csv')
...
def load_electricity_prices(env) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if env.price_data is None:
        ...
        file_path = os.path.join(_data_dir, 'Netherlands_day-ahead-2015-2024.csv')
